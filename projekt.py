...
def wczytaj(nazwa_pliku):
    sciezka = ("input/" + str(nazwa_pliku))
    plik = open(sciezka, encoding="utf-8").read()
    return plik
def main():
    plik = wczytaj("teksty.txt")

    plik = plik.replace("#\n", "</br></br>")
    plik = plik.replace("\n#", "</br></br>")
    plik = plik.replace('\n', ' ')
    plik_csv = wczytaj("semantyka.csv")
    ...
    for tekst in lista_tekstow:
        ...
        waga_tekstu = []
        index_tekst = Tekst("", [])
        index_tekst.id_int = preprocesing.sama_liczba(tekst.split("\n")[0])
        ...
        for slowo in tekst.split(" "):
            if naglowek < 3:
                ...
                podstawowa_forma = preprocesing.podstawowa_forma(preprocesing.samo_slowo(slowo))
                for sem in range(len(semantyka)):
